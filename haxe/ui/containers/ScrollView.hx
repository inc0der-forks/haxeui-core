...
class ScrollView extends InteractiveComponent implements IScroller {
    ...
    @:clonable @:behaviour(DragScrollButton)                        public var dragScrollButton:MouseButton;
...
@:dox(hide) @:noCompletion
@:access(haxe.ui.core.Component)
private class DragScrollButton extends DataBehaviour {
    private var _scrollview:ScrollView;
    ...
    public override function validateData() {
        _scrollview.dragScrollButton = _value;
    }
}
...
class ScrollViewEvents extends haxe.ui.events.Events {
    ...
    private function onLeftMouseDown(event:MouseEvent) {
        if (_scrollview.dragScrollButton == MouseButton.LEFT) {
            onMouseDown(event);
        }
    }
    ...
    private function onMiddleMouseDown(event:MouseEvent) {
        if (_scrollview.dragScrollButton == MouseButton.MIDDLE) {
            onMouseDown(event);
        }
    }
    ...
    private function onRightMouseDown(event:MouseEvent) {
        if (_scrollview.dragScrollButton == MouseButton.RIGHT) {
            onMouseDown(event);
        }
    }
